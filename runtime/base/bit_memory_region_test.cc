...
static void CheckBits(uint8_t* data, size_t size, uint32_t init,
                      size_t offset, size_t length, uintptr_t value) {
...
TEST(BitMemoryRegion, TestBit) {
  uint8_t data[sizeof(uintptr_t) * 2];
  for (size_t bit_offset = 0; bit_offset < 2 * sizeof(uintptr_t)* kBitsPerByte; ++bit_offset) {
...
TEST(BitMemoryRegion, TestBits) {
  uint8_t data[sizeof(uintptr_t) * 4];
  for (size_t bit_offset = 0; bit_offset < 3 * sizeof(uintptr_t)* kBitsPerByte; ++bit_offset) {
    uintptr_t mask = 0;
    for (size_t bit_length = 0; bit_length < sizeof(uintptr_t)* kBitsPerByte; ++bit_length) {
      const uintptr_t value = 0xDEADBEEF0BADF00D & mask;
      for (uint32_t initial_value = 0; initial_value <= 1; initial_value++) {
        // Check Store and Load with bit_offset set on the region.
        std::fill_n(data, sizeof(data), initial_value * 0xFF);
