...
// Returns an acceptable substitution for "a" during MIN/MAX recognition.
static HInstruction* AllowInMinMax(IfCondition cmp, HInstruction* a, HInstruction* b, HInstruction* c) {
  int64_t value = 0;
  if (IsInt64AndGet(b, /*out*/ &value) &&
    ...
    if (other != nullptr) {
      ...
      if (a == c->AsBinaryOperation()->GetLeastConstantLeft() &&
          ((is_max && value >= other_value) ||
           (!is_max && value <= other_value))) {
        return c;
      }
    }
  ...
  if (true_value == false_value) {
    ...
    if (DataType::IsIntegralType(t_type) && DataType::Kind(t_type) == DataType::Kind(f_type)) {
      ...
      if ((cmp == kCondLT || cmp == kCondLE || cmp == kCondGT || cmp == kCondGE) &&
        ...
        HInstruction* thenegated = negLeft ? true_value->InputAt(0) : false_value->InputAt(0);
        HInstruction* notnegated = negLeft ? false_value : true_value;
        if (a == thenegated && a == notnegated && IsInt64Value(b, 0)) {
          // Found a < 0 ? -a :  a
          //    or a > 0 ?  a : -a
