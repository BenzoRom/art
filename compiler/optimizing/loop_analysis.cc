  ...
  for (HBlocksInLoopIterator block_it(*loop_info);
    ...
    for (HBasicBlock* successor : block->GetSuccessors()) {
      if (!loop_info->Contains(*successor)) {
        ...
        // not cause loop peeling to happen as they either can't be inside a loop or by definition
        // can't be loop exits (unconditional instructions) or are non-beneficial for the
        // optimization.
