...
static bool IsSaturatedAdd(HInstruction* a,
  ...
  if (!is_unsigned) {
    ...
    if (IsInt64AndGet(a, &c) || IsInt64AndGet(b, &c)) {
      ...
      //    MIN(r + c,  127) c > 0
      // or MAX(r + c, -128) c < 0 => pick most suitable bound.
      if (0 < c && c <= shi && hi == shi) {
        lo = (lo <= (slo + c)) ? slo : lo;
...
static bool IsSaturatedSub(HInstruction* a,
  ...
  if (!is_unsigned) {
    ...
    if (IsInt64AndGet(a, /*out*/ &c)) {
      // For c in proper range and narrower operand r:
      //    MIN(c - r,  127) c > 0
      // or MAX(c - r, -128) c < 0 => pick most suitable bound.
      if (0 < c && c <= shi && hi == shi) {
        lo = (lo <= (c - shi)) ? slo : lo;
      } else if (slo <= c && c < 0 && lo == slo) {
        hi = (hi >= (c - slo)) ? shi : hi;
...
bool HLoopOptimization::VectorizeSaturationIdiom(LoopNode* node,
  ...
  // Clipped addition or subtraction on narrower operands? We wil try both
