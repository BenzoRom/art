...
void CodeGeneratorARMVIXL::GenerateFrameEntry() {
  ...
  uint32_t spills_offset = frame_size - FrameEntrySpillSize();
  if ((fpu_spill_mask_ == 0u || IsPowerOfTwo(fpu_spill_mask_)) &&
      spills_offset + (fpu_spill_mask_ != 0u ? kArmWordSize : 0u) <= 3u * kArmWordSize) {
    uint32_t core_spills_offset = spills_offset + (fpu_spill_mask_ != 0u ? kArmWordSize : 0u);
    ...
    if (fpu_spill_mask_ != 0u) {
      ...
      GetAssembler()->StoreSToOffset(sreg, sp, spills_offset);
      GetAssembler()->cfi().RelOffset(DWARFReg(sreg), /*offset=*/ spills_offset);
    }
  } else {
    ...
    if (RequiresCurrentMethod() && spills_offset <= 3 * kArmWordSize) {
      DCHECK_EQ(kMethodRegister.GetCode(), 0u);
      __ Push(RegisterList(MaxInt<uint32_t>(spills_offset / kArmWordSize)));
      GetAssembler()->cfi().AdjustCFAOffset(spills_offset);
    } else {
      __ Sub(sp, sp, dchecked_integral_cast<int32_t>(spills_offset));
      GetAssembler()->cfi().AdjustCFAOffset(spills_offset);
...
void CodeGeneratorARMVIXL::GenerateFrameExit() {
  ...
  uint32_t spills_offset = GetFrameSize() - FrameEntrySpillSize();
  if ((fpu_spill_mask_ == 0u || IsPowerOfTwo(fpu_spill_mask_)) &&
      // r4 is blocked for some tests.
      spills_offset + (fpu_spill_mask_ != 0u ? kArmWordSize : 0u) <=
          (blocked_core_registers_[r4.GetCode()] ? 2u : 3u) * kArmWordSize) {
    uint32_t core_spills_offset = spills_offset + (fpu_spill_mask_ != 0u ? kArmWordSize : 0u);
    ...
    if (fpu_spill_mask_ != 0u) {
      ...
      GetAssembler()->LoadSFromOffset(sreg, sp, spills_offset);
      GetAssembler()->cfi().Restore(DWARFReg(sreg));
    }
    uint32_t extra_regs = MaxInt<uint32_t>(core_spills_offset / kArmWordSize) << 2u;
    ...
    GetAssembler()->cfi().RememberState();
    __ Add(sp, sp, spills_offset);
    GetAssembler()->cfi().AdjustCFAOffset(-dchecked_integral_cast<int32_t>(spills_offset));
    if (fpu_spill_mask_ != 0) {
      uint32_t first = LeastSignificantBit(fpu_spill_mask_);
